...
initialize:
    ...
    gh repo create Netbeheer-Nederland/dp-netburen \
        --description "Information models for the Netburen data product." \
        --public \
        --disable-wiki

    git init -b main

    git remote add origin git@github.com:Netbeheer-Nederland/dp-netburen.git
    git fetch
    ...
    gh repo edit Netbeheer-Nederland/dp-netburen --default-branch v0
    git branch -D main
    ...
    #  -H "Accept: application/vnd.github+json" \
    #  -H "X-GitHub-Api-Version: 2022-11-28" \
    #  /repos/Netbeheer-Nederland/dp-netburen/actions/permissions \
    #   -F "enabled=true" -f "allowed_actions=all"

    echo "… OK."
    echo
    echo "A GitHub repository has been created and configured at: https://github.com/Netbeheer-Nederland/dp-netburen"
    echo
...
_post-process-linkml-schema:
    ...
    @if [ -z ${VERSION:-} ]; then \
        sed -i "/^name: .*$/a version: {{shell(ref_name)}}" "artifacts/information_models/dp_netburen.schema.linkml.yml"; \
    else \
...
generate-json-schema: _post-process-linkml-schema
    ...
    @echo "… OK."
    @echo
    @echo "Generated JSON Schema at: artifacts/schemas/json_schema/dp_netburen.json_schema.json"
    @echo
